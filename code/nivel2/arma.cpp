...
arma::arma(int tipo_, QGraphicsScene *scene)
{
    ...
    if(tipo_ == 1)
    {
        cutSprites(":/entidades/textures/level2/gun.png");
        municion = 30;
        vx0 = 180;
        vy0 = 8;
    ...
    {
        cutSprites(":/entidades/textures/level2/lanzagranadas.png");
...
void arma::mover(int x, int y)
{
    if(tipo == 1)
    {
        if(dir == Left)
        {
            setPos(x-6, y + (alto_Jerry - 25)/3);
        }
        else if(dir == Right)
        {
            setPos(x+28, y + (alto_Jerry - 25)/3);
        }
        else setPos(x, y + (alto_Jerry - 25)/3);
    }
    if(tipo == 2)
    {
        if(dir == Left)
